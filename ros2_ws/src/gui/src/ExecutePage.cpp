    ...
    , ui(new Ui::FormHau)
{
    ...
    "/fake_Force", 100,
    std::bind(&FormHau::topic_callback_Force, this, std::placeholders::_1));

    torque_subscription_ = node->create_subscription<std_msgs::msg::Int64>(
    "/fake_Torque", 100,
    std::bind(&FormHau::topic_callback_Torque, this, std::placeholders::_1));

    spindle_subscription_ = node->create_subscription<std_msgs::msg::Int64>(
    "/fake_Spindle", 100,
    std::bind(&FormHau::topic_callback_Spindle, this, std::placeholders::_1));
    ...
    connect(ui->start_process_btn, &QPushButton::clicked, this, &FormHau::start_process);
    connect(ui->stop_process_btn, &QPushButton::clicked, this, &FormHau::stop_process);
}

void FormHau::callback(const rclcpp::Client<gui_msgs::srv::AutoProcess>::SharedFuture result)
{
...
void FormHau::start_process()
{
    ...
    request->req = 1;
    auto result = client->async_send_request(request, std::bind(&FormHau::callback, this, std::placeholders::_1));
}

void FormHau::stop_process()
{
...
void FormHau::topic_callback_Force(const std_msgs::msg::Int64::SharedPtr msg)
{
    // RCLCPP_INFO(node->get_logger(), "Received joint state: %f", msg->position[0]);
    this->time_series_plot_Force->append_graph(msg->data);
}

void FormHau::topic_callback_Torque(const std_msgs::msg::Int64::SharedPtr msg)
{
    this->time_series_plot_Torque->append_graph(msg->data);
}

void FormHau::topic_callback_Spindle(const std_msgs::msg::Int64::SharedPtr msg)
{
...
void FormHau::openRviz(QWidget* shared_rviz_widget)
{
